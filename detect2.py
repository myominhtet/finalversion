...
def is_scanner_moving(prev_centroids, curr_bbox, track_id, threshold=5.0):
    """Determine if a scanner is moving based on centroid displacement."""
    x, y, w, h = curr_bbox
    curr_centroid = (x, y)  # Center of the bbox (x, y)
...
def detect(save_img=False):
    ...
    # Dictionary to store previous centroids for each track ID
    prev_centroids = {}
            ...
            if len(det):
                ...
                if save_img or view_img:
                    for track in tracker.tracks:
                        ...
                        movement_status = ""
                        if class_name.lower() == "scanner":
                            is_moving = is_scanner_moving(prev_centroids, bbox, track.track_id, threshold=5.0)
                            movement_status = "Scanning" if is_moving else "Idle"
                            # Update previous centroid
                            prev_centroids[track.track_id] = (x, y)
